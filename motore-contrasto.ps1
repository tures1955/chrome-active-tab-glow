...
public static class Contrasto {
    ...
    // Se il rettangolo richiesto sconfina anche solo un po' fuori dai
    // bordi veri dell'immagine sorgente (puo' succedere se Chrome si sta
    // ancora "assestando", ad esempio appena dopo l'accensione del PC),
    // GDI+ non si limita a ritagliare quello che c'e': lancia un errore
    // "memoria insufficiente" fuorviante. Meglio limitarsi sempre alla
    // zona che esiste davvero.
...
$timer.Add_Tick({
    try {
        ...
        # Se non abbiamo (piu') una finestra Chrome valida -- es. il motore
        # e' partito PRIMA di Chrome all'accensione del PC, e da allora non
        # hai mai dato il focus vero a Chrome col mouse -- ricerchiamo da
        # capo una finestra Chrome visibile, ad ogni ciclo, finche' non la
        # troviamo. Cosi' non serve nessuna interazione manuale.
